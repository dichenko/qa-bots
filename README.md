...
Для бота с токеном из `TELEGRAM_BOT_TOKEN` (бот по умолчанию) вебхук будет установлен на:
```
<VERCEL_URL>/api/index
```

Для дополнительных ботов с токенами из `TELEGRAM_BOT_TOKEN_<ID_БОТА>` вебхуки будут установлены на:
```
...
> **Важно**: Запросы на путь `/api/index` всегда обрабатываются ботом по умолчанию из переменной `TELEGRAM_BOT_TOKEN`. Если эта переменная не задана, но есть другие боты, запросы будут обрабатываться другим доступным ботом.
